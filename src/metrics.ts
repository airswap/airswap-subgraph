...
export function updateDailySwapVolume(event: SwapEvent, swapValue: BigDecimal): void {
...
export function updateDailyFeesCollected(event: SwapEvent, feeValueUsd: BigDecimal): void {
    //the following uses integer division based on the number of seconds in a day to generate the id and date
    let dayId = event.block.timestamp.toI32() / 86400
    let dayStartTimestamp = dayId * 86400
